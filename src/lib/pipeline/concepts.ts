...
PRODUCT STAGING (non-negotiable): the image model renders the REAL product from its reference photograph directly into your scene, so every scene MUST contain the product, staged and used correctly. Vary how it is staged across the set — a clean styled studio surface for one concept, a lived-in human scene (hands, a family meal, the finished dish on the table) for another — but never write a scene that omits the product, and never describe it twice in one frame.

ONE PHOTOGRAPH, ONE MOMENT: an image model renders a single frame. Never brief a triptych, a split screen, a before/after, a storyboard, a grid, or "the same product across three days" — those render as broken collages. One scene, one moment, one point of view.
...
4. imagePrompt is THE complete final prompt for a photoreal image model. It must single-handedly carry:
   ...
   - the real product staged prominently, naturally and CORRECTLY (right placement, right usage, never a redesigned pack) — and for APPAREL concepts the garment WORN by an aspirationally styled person, premium lookbook energy, exact colour/pattern/cut preserved;
